...
public class BookValidator : Validator<Book>
{
    protected override void InitializeValidations()
    {
        ...
        Validations.Add("Author is required.", book => !string.IsNullOrEmpty(book.Author) && book.Author.All(c => char.IsLetter(c) || char.IsWhiteSpace(c)));
        Validations.Add("ISBN is required.", book => !string.IsNullOrEmpty(book.ISBN));
        Validations.Add("Genre is required.", book => !string.IsNullOrEmpty(book.Genre) && book.Genre.All(c => char.IsLetter(c) || char.IsWhiteSpace(c)));
